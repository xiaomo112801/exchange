...
const allRecords = ref<WithdrawRecord[]>([
  {
    id: 1,
    coinCode: 'USDT',
    coinName: 'Tether',
...
function handleRecordClick(record: WithdrawRecord) {
  // 可以跳转到详情页
...
<template>
  <view class="withdraw-record-page min-h-screen flex flex-col bg-white">
    <!-- 顶部导航栏 -->
    <wd-navbar
      safe-area-inset-top
      placeholder
    ...
    <view class="flex flex-1 flex-col px-4 pb-3 pt-3">
        ...
        >
          ...
          <view class="flex items-center justify-between border-t border-gray-100 pt-2">
            ...
            <view class="flex flex-col items-end">
              <wd-text text="提现地址" size="0.75rem" color="#999" />
          ...
          <!-- 手续费 -->
          <view class="flex items-center justify-between border-t border-gray-100 pt-2">
            <wd-text text="手续费" size="0.75rem" color="#999" />
            <wd-text :text="`${record.fee} ${record.coinCode}`" size="0.75rem" color="#666" />
          </view>
      ...
      <view v-else class="flex flex-1 items-center justify-center">
        <view class="flex flex-col items-center gap-2">
          ...
          <wd-text text="暂无提现记录" size="0.875rem" color="#999" />
        </view>
      </view>
    </view>
  </view>
